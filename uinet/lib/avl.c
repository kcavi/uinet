...
avl_add(avl_tree_t *tree, void *new_node)
{
	...
	{
	#if 0
		printf("avl_find() succeeded inside avl_add()\r\n");
	#endif
		return;
	}
...
avl_remove(avl_tree_t *tree, void *data)
{
	...
	delete1 = AVL_DATA2NODE(data, off);
	 ...
	 */
	if (delete1->avl_child[0] != NULL && delete1->avl_child[1] != NULL) 
	{
		/*
		 * choose node to swap from whichever side is taller
		 */
		old_balance = AVL_XBALANCE(delete1);
		 ...
		 */
		for (node = delete1->avl_child[left];
		    node->avl_child[right] != NULL;
		    node = node->avl_child[right]);
		...
		*node = *delete1;
		 ...
		 */
		delete1 = &tmp;
		parent = AVL_XPARENT(delete1);
		parent->avl_child[AVL_XCHILD(delete1)] = delete1;
		which_child = (delete1->avl_child[1] != 0);
		if (delete1->avl_child[which_child] != NULL)
			AVL_SETPARENT(delete1->avl_child[which_child], delete1);
	}
	...
	--tree->avl_numnodes;
	parent = AVL_XPARENT(delete1);
	which_child = AVL_XCHILD(delete1);
	if (delete1->avl_child[0] != NULL)
		node = delete1->avl_child[0];
	else
		node = delete1->avl_child[1];
...
void *
avl_greater(avl_tree_t *tree, const void *value)
{
	avl_index_t where;
	void *node = NULL;

	node = avl_find(tree, value, &where);
	if (node != NULL) 
	{
	...
	{
		node = avl_nearest(tree, where, AVL_AFTER);
	}
...
void *
avl_less(avl_tree_t *tree, const void *value)
{
	avl_index_t where;
	void *node = NULL;
	...
	if (node != NULL) 
	{
		node = AVL_PREV(tree, node);
	}
	else 
	{
		node = avl_nearest(tree, where, AVL_BEFORE);
	}
...
void *
avl_greater_equal(avl_tree_t *tree, const void *value)
{
	avl_index_t where;
	void *node = NULL;
...
void *
avl_less_equal(avl_tree_t *tree, const void *value)
{
	avl_index_t where;
	void *node = NULL;
